...
# ==========================================
# 2. ฟังก์ชันโหลดข้อมูลเวอร์ชันซ่อมแซมทางเดินไฟล์ 🌟
# ==========================================
@st.cache_data
def load_data():
    # วิธีที่ 1: ค้นหาไฟล์โดยตรงในโฟลเดอร์ทำงานปัจจุบัน
    ...
    for filename in possible_files:
        if os.path.exists(filename):
            try:
                ...
                return df, f"ในระบบ ({filename})"
            except:
                continue

    # วิธีที่ 2: ดึงผ่าน URL โดยตรงจาก GitHub (แผนสำรองป้องกันระเบิด)
    try:
        ...
        df = df.dropna()
        df["number"] = df["number"].astype(float).astype(int).astype(str).str.zfill(2)
        return df, "ลิงก์ออนไลน์ (GitHub URL)"
    except:
        pass
...
if df is not None:
    st.toast(f"🐣 เปิดคัมภีร์สำเร็จ: ดึงจาก {file_used}", icon="✅")
    ...
    st.error("😭 แงงง น้อนพยายามหาทุกทางแล้วก็ยังไม่เจอไฟล์เลย")
    st.info("กรุณาเช็กให้แน่ใจว่าชื่อไฟล์บน GitHub ของคุณคือ to_lotto.csv (พิมพ์เล็กทั้งหมด) และอยู่ในหน้าแรกสุดคู่กับ ruaydee.py นะครับ")