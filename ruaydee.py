...
# ==========================================
# 2. ฟังก์ชันโหลดข้อมูลแบบ ปรับปรุงใหม่เพื่อไฟล์ของคุณ 👑
# ==========================================
@st.cache_data
def load_data():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    possible_files = ["to_lotto.csv", "to_lotto.xlsx", "to_lotto_2.xlsx"]
    ...
    for filename in possible_files:
        ...
        if os.path.exists(path):
            try:
                ...
                # ลบแถวที่เป็นค่าว่างทั้งหมดออกไปก่อน
                df = df.dropna(how='all')
                
                # บังคับเปลี่ยนชื่อคอลัมน์ให้โค้ดทำงานต่อได้ ไม่ว่าหัวตารางจะเป็นภาษาอะไร
                df.columns = ["date", "number"]
                
                # ล้างแถวที่ข้อมูลไม่ครบอีกที
                df = df.dropna()
                
                # แปลงตัวเลขหวยให้เป็นข้อความ 2 หลัก (เช่น เลข 5 เป็น "05")
...
# หัวข้อหลักสุดน่ารักพร้อมน้อนกระต่ายและน้อนหมีพ่นเงิน 
st.markdown(
    """
...
if df is not None:
    ...
    with col1:
        # ปุ่มสุ่มดวงดาว (ธีมน้อนแมวอวกาศ)
    ...
    with col2:
        # ปุ่มสแกนสถิติ (ธีมน้อนหมีนักคำนวณ)
    ...
    st.error("😭 แงงง ไม่พบไฟล์ข้อมูลหวยใน GitHub เลย")
    st.info("พี่ๆ อย่าลืมอัปโหลดไฟล์ `to_lotto.csv` ไว้ในโฟลเดอร์เดียวกับโค้ดตัวนี้นะน้าา น้อนหาไม่เจอ 🥺")